<template>
  <div>
    <div class="class1" @mouseenter="enterm" @mouseleave="leavem">
      <span
        class="div-body-data-table-dl-dt-del"
        v-if="ItemData.el.isSelected"
      >{{ItemData.index + 1}}.</span>
      <span v-else>{{ItemData.index + 1}}.</span>
      <Checkbox
        v-model="ItemData.el.isSelected"
        size="large"
        class="div-body-data-table-dl-dt-checkbox"
        style="position:relative"
        @on-change="clickTese(ItemData.index)"
      >
        <span v-if="!ItemData.el.isEditing" @dblclick="editName(ItemData.index)">
          <del
            class="div-body-data-table-dl-dt-del"
            v-if="ItemData.el.isSelected"
          >{{ItemData.el.itemName}}</del>
        ...
        <Input
          v-else
          v-model="ItemData.el.itemName"
          :autofocus="true"
          class="div-body-data-table-dl-dt-input"
          size="small"
          @on-blur="itemOnBlur(ItemData.index)"
          @on-enter="itemOnBlur(ItemData.index)"
        />
