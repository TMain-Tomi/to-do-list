<template>
    <div >
      <div class="class1" @mouseenter="enterm" @mouseleave="leavem">
        ...
        <Checkbox v-model="ItemData.el.isSelected" size="large" class="div-body-data-table-dl-dt-checkbox" style="position:relative" @on-change="clickTese(ItemData.index)" >
            ...
            <Input v-else v-model="ItemData.el.itemName" :autofocus="true" class="div-body-data-table-dl-dt-input" size="small" @on-blur="itemOnBlur(ItemData.index)" @on-enter="itemOnBlur(ItemData.index)"/>
             <button class="display" v-bind:class="{display1}" @click="remove(ItemData.index)">&times;</button>
        </Checkbox>
           </div>
    </div>
</template>
